  ...
  return (
    <View>
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.8}
      >
        <Text className={`font-jarkatamedium text-base ${selected ? 'text-gray-800' : 'text-gray-400'}`}>
      ...
      >
        <BottomSheetContainer title={label} onClose={closeSheet}>
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                className="w-full flex-row items-center justify-between py-4 px-2 border-b border-gray-100"
                onPress={() => {
                  onChange(item.value);
                  closeSheet();
                }}
